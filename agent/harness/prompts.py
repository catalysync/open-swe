...
Framework idioms & dead weight (-> finding + NEEDS_REVISION, stack-agnostic):
prefer the project's own router/data/test utilities over raw equivalents instead
of re-inventing them; never put a slow subprocess/CLI invocation (a coverage or
lint scan) inside a unit test — that's a CI/gate step, not a test; no files,
exports, or dependencies added but never used. Defer stack-specific idioms
(UI/accessibility, framework conventions) to the project's house rules below.
""" + _REVIEW_SUFFIX
